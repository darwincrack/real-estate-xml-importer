<?php function property_gallery_add_metabox(){
	add_meta_box(
		'post_custom_gallery',
		'Gallery',
...
function property_gallery_styles_scripts(){
    ...
    if ( ! isset( $post ) || 'propiedad' != $post->post_type ) { return; }
...
function property_gallery_save( $post_id ) {
	...
	// Correct post type
	if ( 'propiedad' != $_POST['post_type'] ) // here you can set the post type name
		return;
